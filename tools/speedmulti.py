...
from loader import LatLontoUTM, RadToDeg, NearestWayPointCTEandDistance,\
    total_lap_distance
...
def generateImagesFromPaths(data, batchSize, inputShape, outputShape, train = False):
    ...
    while 1:
        ...
        yield({'inputImg': returnArr,'inputSpeed': np.array(speedArr)},
              {'outputSteer': labels[:,0],'outputThr': labels[:,1]-labels[:,2], 'outputPos': np.array(vecArr)}, [weights,weights])
...
def main():
    ...
    data9 = pd.read_csv('/home/jjordening/data/4441144/outputNew.txt')
    data9['path'] = '/home/jjordening/data/4441144/'+data9['path'].apply(lambda x: x.strip())
    print('data9', np.max(data9['steering']), np.min(data9['steering']))
    dataList.append(data9)
    
    data10 = pd.read_csv('/home/jjordening/data/4441146/outputNew.txt')
    data10['path'] = '/home/jjordening/data/4441146/'+data10['path'].apply(lambda x: x.strip())
    print('data10', np.max(data10['steering']), np.min(data10['steering']))
    dataList.append(data10)
    
    data11 = pd.read_csv('/home/jjordening/data/4441147/outputNew.txt')
    data11['path'] = '/home/jjordening/data/4441147/'+data11['path'].apply(lambda x: x.strip())
    print('data11', np.max(data11['steering']), np.min(data11['steering']))
    dataList.append(data11)
    
    data12 = pd.read_csv('/home/jjordening/data/4441148/outputNew.txt')
    data12['path'] = '/home/jjordening/data/4441148/'+data12['path'].apply(lambda x: x.strip())
    ...
    stopCallback = EarlyStopping(monitor='val_loss', patience = 10, min_delta = 0.01)
    ...
    if LOADMODEL:
        endModel = load_model('psyncModelSpeed.h5', custom_objects={'customLoss':customLoss})
        ...
        endModel.save('psyncModelSpeed.h5')
        ...
        xOutPos = Dense(50, name='thr1')(xEnd)
        xOutPos = BatchNormalization(name='thr2')(xOutPos)
        xOutPos = Activation('elu')(xOutPos)
        xOutPos = Dropout(.3)(xOutPos)
        xOutPos = Dense(10, name='thr3')(xOutPos)
        xOutPos = BatchNormalization(name='thr4')(xOutPos)
        xOutPos = Activation('elu')(xOutPos)
        xOutPos = Dense(1, activation='sigmoid',name='thr5')(xOutPos)
        xOutPos = Lambda(lambda x: x*2-1, name = 'outputThr')(xOutPos)
